...
(define (CRT-exists? cong-sys)
  (if(null? (cdr cong-sys))
     #t
     (if(= (euclid-gcd(cadar cong-sys) (m (cdr cong-sys)))1)
           (CRT-exists? (cdr cong-sys))#f)))
...
(define (CRT-helper cong-sys m)
  (if( null? cong-sys)
     0
     (+(*(*(caar cong-sys)(/ m (cadar cong-sys)))(mul-inv(/ m (car(cdr(car cong-sys))))(cadar cong-sys)))(CRT-helper (cdr cong-sys) m))))
