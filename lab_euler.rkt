...
(define (get-vertices G)
  (cond ((null? G)'())
        (else (usort(append (cons(caar G) (cdar G)) (get-vertices (cdr G)))))))
...
(define (degree G v)
  (cond ((null? G) 0)
        ...
        ((eq? (cadar G) v) (+ 1 (degree (cdr G) v)))
        ((eq? (caar G) v) (+ 1 (degree (cdr G) v)))
        (else (degree (cdr G) v))))
...
(define (all-even? G)
  (define (evenHelper? G D)
    (cond ((null? D) #t)
          ((eq? 0 (modulo(degree G (car D)) 2)) (evenHelper? G (cdr D)))
          (else #f)))
  (evenHelper? G (get-vertices G)))
...
(define (adjacent G v)
  (define (adjHelper G v)
    (cond ((null? G) '())
          ((eq? (cadar G) v) (cons(caar G) (adjHelper (cdr G) v)))
          ((eq? (caar G) v) (cons(cadar G) (adjHelper (cdr G) v)))
          (else (adjHelper (cdr G) v))))
  (usort (adjHelper G v)))
...
(define (connected? G)
  (equal? (get-vertices G) (usort(connectedHelper G))))

(define (connectedHelper G)
  (if(null? G) '() (dfs G '()(caar G))))
...
(define (eulerian? G)
  (cond ((null? G) #t)
        ((and (eq? (connected? G) #t) (eq? (all-even? G) #t)) #t)
        (else #f)))