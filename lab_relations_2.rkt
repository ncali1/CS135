...
(define (compose RO RI)
   (cond ((null? RI) '())
         (else (make-set (append (composeHelper RO RI) (compose RO (cdr RI)))))))



(define (composeHelper RO RI)
  (cond ((null? RO)'())
        ((eq?  (cadar RI) (caar RO)) (cons (list (caar RI) (cadar RO)) (composeHelper (cdr RO) RI)))
        (else (composeHelper (cdr RO) RI))))
...
(define (powerHelper R k store)
  (if (= k 0)'()
      (if (= k 1)
          store
      (powerHelper R (- k 1)(compose R store)))))
...
(define (transitive-closureHelper R k keep)
  (if (subset? (power R k) keep)
      keep
      (transitive-closureHelper R(+ k 1) (union (power R k) keep))))
...
(define (EQ-relation? R n)
  (and(and(reflexive? R n) (symmetric? R) (transitive? R))))
