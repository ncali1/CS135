...
(define (mod-exp b e m)
  (cond ((zero? e) 1) 
        ((zero? (modulo e 2)) (modulo (expt (mod-exp b (/ e 2) m) 2) m))
...
(define (encrypt plaintext key)
  (cond((null? (string->list plaintext)) '())
         (else(cons(mod-exp(char->integer (car (string->list plaintext))) (cadr key)(car key)) (encrypt (substring plaintext 1) key)))))
...
(define (decryptHelper ciphertext key)
  (cond ((null? ciphertext) '())
        (else (cons(integer->char (mod-exp(car ciphertext) (cadr key)(car key))) (decryptHelper(cdr ciphertext) key)))))
