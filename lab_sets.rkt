...
(define (intersection LA LB)
  (if (null? LB) '()
      (if (element? (car LB) LA) (make-set(append (list (car LB)) (intersection LA (cdr LB))))
...
(define (subset? SA SB)
  (equal?(length (intersection SA SB)) (length (make-set SA))))
...
(define (set-difference LA LB)
  (if(null? LA) '()
     (if (element? (car LA) LB) (set-difference(cdr LA) LB)
      (make-set(append (list (car LA)) (set-difference(cdr LA) LB))))))   
...
(define (sym-diff LA LB)
  (make-set(append(set-difference LA LB)(set-difference LB LA))))
